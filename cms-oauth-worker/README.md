...
הקריאה ל-Gemini הוסרה לגמרי (הייתה מוגבלת ל-20 בקשות ביום בתוכנית
החינמית). זיהוי הטקסט מהצילום עצמו קורה עכשיו **בדפדפן** בלבד, באמצעות
Tesseract.js (OCR מקומי, ללא עלות ובלי מכסה) - סכום, תאריך ואסמכתה
מחולצים ב-regex בצד הלקוח; שם/טלפון/מייל אף פעם לא מנוחשים מהתמונה.

`POST /bit-receipts/extract` מקבל רק טקסט (`{ description }` - התוצר של
ה-OCR בדפדפן, לא תמונה) ומחפש בו התאמה ל-`descriptionKeyword` של לקוח
שמור - **אף פעם לא לפי שם הלקוח עצמו**, כי אין קשר מובטח בין השם לבין
מה שהלקוח בפועל כותב בהערת ההעברה. שני שלבים: קודם הביטוי המלא של
מילת הזיהוי (למשל "עבודה עם דוד"), ואם זה לא נמצא - כל מילה משמעותית
בודדת מתוכו (כלומר גם אם נכתב רק "דוד" ולא הביטוי המלא). מילות קישור
קצרות (עם/של/את/גם...) לא נספרות. אם שני לקוחות מתאימים באותה רמה, לא
מנחשים - מוחזר `candidates` עם הרשימה המצומצמת במקום `matchedContact`
שגוי. **לא כותב שום דבר ל-KV ולא פונה לשום שירות חיצוני** - מחזיר לכל
היותר רשומת לקוח בודדת שהתאימה (`matchedContact`), לעולם לא את הרשימה
המלאה, כך שבטוח לקרוא לו מהטופס הציבורי ללא הרשאה. מוגבל ל-8 בקשות
לחלון rate-limit לפי IP.
